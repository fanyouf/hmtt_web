...
情况1：草根公司。前端打包（/dist） 交给后端，你就不用管了。

情况2：有自动布署的代码功能: 只需要push代码，自动打包，发布 ...... （github actions）

情况3：有自已的布署发布的系统。

- 公司内网去申请主机（配置：内存，硬盘，几核，数据库......）
- 只上传源文件(./src)---就是push代码。 它会自动去运行npm run build,把打包后的文件放置指定的位置....