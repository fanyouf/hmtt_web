...
![image-20210228094122272](asset/image-20210228094122272.png)

![image-20210228094341383](asset/image-20210228094341383.png)
...
- 是生产依赖（项目发布上线了还是需要vant的），而不是开发依赖（只是在开发项目时使用，而上线了就不需要）
...
因为这个项目是移动端（用户是通过手机来访问的），所以，它需要去适配不同的手机屏幕。我们希望实现适配的效果是：与屏幕大小相关。以按钮为例：**在大屏手机，按钮的宽高都大些，在小屏手机上宽高都小些**。
...
设置基地址：baseURL: 'http://toutiao-app.itheima.net'
