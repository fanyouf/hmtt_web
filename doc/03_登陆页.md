...
<img src="asset/image-20200606235656399.png" alt="image-20200606235656399" style="zoom:50%;" />
...
methods: {
  hLogin () {
    ...
    ajax({
        ...
        // 参数: 根据接口文档，参数要通过body（请求体中）来传，对应到axios中，用data
        data: {
            mobile: this.mobile,
            code: this.code
        }
...
async hLogin () {
    ...
    this.$toast.loading({
        duration: 0, // 持续展示 toast,永远不会关闭
        // node_modules\vant\lib\toast\index.js 中可以发现mask是一个过时的设置（它还是可以生效），改成overlay
        // mask: true, // 整体添加一个遮罩
        overlay: true,
        message: '登陆中...'
    })
