...
```javascript
  ...
  <script>
    ...
    // 下面的代码的输出结果是： a<span>b</span>cdefgbefab
    // 它只能替换一次，不能全部替换
    // console.log(str.replace('b','<span>b</span>') )
...
- 使用一个**计算属性**来保存高亮之后的值
  - 原来直接获取的数据要做一次加工：用正则replace对内容进行高亮处理---字符串替换
- 用v-html显示数据
...
<img src="asset/image-20200612142517738.png" alt="image-20200612142517738" style="zoom:50%;" />
