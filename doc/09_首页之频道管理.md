...
这里的数据在Index.vue中的频道列表中就已经获取过的。 可以从home/index.vue ---> home/channelEdit.vue （典型的组件父传子）
...
![image-20200710102127229](asset/image-20200710102127229.png)
...
![image-20200611120910511](asset/image-20200611120910511.png)
...
​	哪一个频道处于激活状态是由index.vue组件中 `activeIndex`决定的。所以，只需把这个值传给子组件即可。
...
```
/**
 * 间接实现添加订阅频道的功能。 把当前用户选中的所有频道全部传入（不包含 推荐 频道）
 * @param {*} channels 用户当前的订阅频道
 * [{id: 频道1id, seq: 1 }, {id: 频道2id, seq: 2 }, .....]
 * 不包含 推荐 频道
 */
...
用户登陆，点击某个推荐频道，则会把这个频道信息加入到用户订阅频道中去，刷新页面可以在频道列表中看到效果。
...
hTest () {
   ...
   // this.channels = [{ id: 100, name: 'test' }] // 不能改

   // this.channels.push({ id: 100, name: 'test' }) // ok
...
![image-20200710151243666](asset/image-20200710151243666.png)
