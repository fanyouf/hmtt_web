...
- 对需要权限才能访问的页面进行保护：如果用户没有登陆则不让访问
- token过期时进行无感操作：重新获取新token
...
当我们发现一个没有登陆的用户试图去访问一个需权限的页面时，我们应该：

- 把它引导到登陆界面
- 当用户完成登陆操作之后，再跳回到他之间访问的页面。
